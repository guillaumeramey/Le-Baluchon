...
class ChangeViewController: UIViewController {
    ...
    private func setupDesign() {
        for textField in amounts {
            textField.layer.cornerRadius = 5.0
            textField.layer.borderColor = UIColor(named: "Color_bar")!.cgColor
            textField.layer.borderWidth = 1
        }
    }

    private func updateRatesIfNeeded() {
    ...
    private func updateRates() {
        ...
        ChangeService.shared.getRates { (success, changeJSON) in
            if success, let changeJSON = changeJSON {
                self.change = Change(date: changeJSON.date, rates: changeJSON.rates)
                self.updateDisplay()
